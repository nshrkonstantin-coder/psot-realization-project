...
def _verify_session(event: dict) -> dict | None:
    """Проверяет сессионный токен из заголовков или query-параметра token=."""
    ...
    if method == 'OPTIONS':
        return {
            ...
            'headers': {
                ...
                'Access-Control-Allow-Headers': 'Content-Type, X-User-Id, X-User-Role, X-Auth-Token, Authorization',
