      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Дополнительные направления</h1>
              <p className="text-slate-400 text-sm mt-1">Отдел ОТиПБ</p>
            </div>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {additionalDirections.map((direction, index) => (
            <Card
              key={index}
              onClick={() => navigate(direction.route)}
            ...
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${direction.color} opacity-0 group-hover:opacity-10 transition-opacity`} />
              
              <div className="p-8 relative z-10">
                <div className="flex flex-col items-center text-center gap-4">
                  <div className={`bg-gradient-to-br ${direction.color} p-6 rounded-2xl shadow-lg transform group-hover:scale-110 transition-transform`}>
                    <Icon name={direction.icon} size={40} className="text-white" />
                  </div>
                  <div>
...
export default OtipbAdditionalDirectionsPage;
